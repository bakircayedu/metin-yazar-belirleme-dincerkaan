  ...
  public   class Heap
    {
        ...
        public bool Insert(string value)
        {
            ...
            {
                if (heapArray[i].value.Kelimee == value)
                {
                    heapArray[i].value.Frekans++;
                    MoveToUp(i);
                    return true;
                }
            }
        ...
        public void MoveToUp(int index)
        {
            ...
            while (index > 0 && heapArray[parent].value.Frekans < bottom.value.Frekans)
            {
        ...
        public void MoveToDown(int index)
        {
            ...
            while (index < currentSize / 2)
            {
                ...
                //Find larger child
                if (rightChild < currentSize && heapArray[leftChild].value.Frekans <
                heapArray[rightChild].value.Frekans)
                    largerChild = rightChild;
                else
                    largerChild = leftChild;
                if (top.value.Frekans >= heapArray[largerChild].value.Frekans)
                    break;
        ...
        public string Disp()
        {
            ...
            for (int i = 0; i < currentSize; i++)
            {
                sonuc += heapArray[i].value.Kelimee + "--"+ heapArray[i].value.Frekans + "\n";
            }
